...
for game in range(starting_game, starting_game + num_game):
    ...
    model.fit(x=data['input'], y=data['output'], batch_size=batch_size, epochs=(game+1)*config.training_param['EPOCHS'], verbose=1, initial_epoch=game*config.training_param['EPOCHS'])
...
if len(log_weights) > 0:
    log_weights.sort(key=lambda f: int(''.join(filter(str.isdigit, f))) or -1)
    copyfile(os.path.join(log_folder,log_weights[-1]), os.path.join(save_model,'current_final_weights.h5'))
