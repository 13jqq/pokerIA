...
class RLPLayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        if self.ismcts == None or state.id not in self.ismcts.tree:
    ...
    def buildISMCTS(self, state):
        self.root = im.Node(state)
        self.ismcts = im.ISMCTS(self.root, self.cpuct)

    def changeRootISMCTS(self, state):
        self.ismcts.root = self.ismcts.tree[state.id]
